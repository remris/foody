...
class IngredientCatalog {
  ...
  /// Sucht nach einem Katalog-Eintrag anhand des Namens (exakt oder Alias).
  /// Gibt null zurück wenn kein Eintrag gefunden wurde.
  static IngredientEntry? findByName(String name) {
    final lower = _normalize(name.toLowerCase().trim());
    for (final entry in all) {
      if (_normalize(entry.name.toLowerCase()) == lower) return entry;
      for (final alias in entry.aliases) {
        if (_normalize(alias.toLowerCase()) == lower) return entry;
      }
    }
    // Fallback: enthält-Suche
    for (final entry in all) {
      if (_normalize(entry.name.toLowerCase()).contains(lower) ||
          lower.contains(_normalize(entry.name.toLowerCase()))) {
        return entry;
      }
    }
    return null;
